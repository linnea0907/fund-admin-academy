...
- ~~**P3-甲**~~：`check-glossary.mjs` 显式跳过 `imported.ts`（`f !== "imported.ts"`），
  即导入术语**不走内建的 14 字段闸门**。 → **V1.20.6 已收口**（闸门改为直载 `GLOSSARY_TERMS`，导入 ≈ 内置同一套六项校验）。
- ~~**P3-乙**~~：「已发布术语」视图与「术语列表」页签、健康度 Dashboard 有信息重叠。 → **V1.20.6 已删**（按 Subtraction First 移除）。
- **P3-丙**：词组档精度约六成（人名 / 通用词混入）。更强的收紧手段会误杀只出现一次的真术语。
  → V1.20.6 以「默认只看高置信」缓解（不收紧规则本身）。

链路规范全文见 **`docs/TERM-AUTODISCOVERY-SPEC.md`**。

---

## V1.20.6 已交付（来源：2026-09-20 Lu 直接下达 · 术语自动发现收敛 P1）

四项建议一次落地，全部围绕 V1.20.5 的术语自动发现链路做**收敛**，无新增模块。

### 需求 1：导入术语纳入 14 字段闸门（收口 P3-甲）

**根因比原判断更深**：闸门跳过 `imported.ts` 不只是「它是生成物」——
该文件由 `JSON.stringify` 生成，**key 带双引号**（`"id": "x"`），而旧闸门的正则是
`\s{4}id:\s*"..."`，**一条都匹配不到**。即使把文件加回列表，也会「0 命中、静默通过」（假绿）。

**做法**：闸门不再用正则解析 TS 源码，改为经 `scripts/lib/ts-loader.mjs` **直载**
`GLOSSARY_TERMS`（内置 ∪ `IMPORTED_TERMS`）。六项检查对导入条目**一视同仁**：
重复 id / 断链 / 案例引用 / 课程引用 / 标注撞车 / 必填字段。报错信息标注来源
（`内置` / `imported.json`），便于定位坏数据来自哪条链路。

**配套**：`prebuild` 顺序调整为 `check:lesson-numbers → gen:cases → gen:glossary → check:glossary → scan:terms`
（`gen:glossary` 必须先跑，否则校验的是上一版烘焙产物）。

### 需求 2：简化术语审核页（收口 P3-乙）

- **删除「已发布术语」视图**（`PublishedTable`，71 行）+ `data-published` 锚点，
  并移除 `terms` / `usageCounts` 两个 props 的透传 —— 与「术语列表」页签重复，按 Subtraction First 移除。
- 分段由「待审核 / 已发布」改为**审核三态**：**待审核 / 已采纳 / 已忽略**（互斥）。
  「已忽略」不再是切换按钮，升为独立分段。
- **同时删除 4 张统计卡** —— 数字已在分段徽标与置信 chip 上重复出现，属同页冗余。

### 需求 3：候选池默认按置信度过滤

- 新增**「高置信」合并档**（`declared` ∪ `acronym`）并设为**默认**。
- 筛选 chip 顺序：`高置信 42`（默认）/ `全部 115` / `正文声明 11` / `缩写 31` / `词组 73`。
  词组档需显式切换，避免噪声淹没审核队列。
- 待审核计数与列表筛选**解耦**（计数只认「未忽略且未采纳」），切换筛选不会让徽标跳动。

### 需求 4：术语补全包升级为「结构化半成品」（不接入模型）

**口径（Lu 定稿）**：**系统负责发现，AI 负责内容**。审核页只做发现 / 审核 / 采纳 / 忽略 / 导出，
**不在站内生成或编辑术语正文** —— 避免出现两个内容来源造成后续同步与覆盖风险。
因此**不接入任何模型能力**，内容生产留在 Copilot 侧。

补全包现在分两类字段：

| 谁填 | 字段 |
|---|---|
| **系统预填（零臆造）** | `id` / `term` / `zh` / `fullName` / `courses` / `cases` |
| **Copilot 撰写** | `definition` / `whyImportant` / `commonMistakes` / `brief` / `category` / `level` / `jurisdiction` / `scenario` / `aliases` / `related` / `source` / `tags` |

**关键发现**：扫描器此前**丢掉了正文自带的真实声明** —— `Private Investment Fund（私募投资基金）`
的中文名、`SAR（Suspicious Activity Report）` 的英文全称，都被 `collect(rx, conf, 1)`
只取了缩写而丢弃。V1.20.6 补回 `zh` / `fullName`，并新增课程 id 映射（`courseIds`），
使这三类字段可直接填**真实值**。

**防呆**：`imported.json` 中任何字段值含占位符 `〔待补〕` → 生成器**拒收该条并告警**；
`related` 未替换还会被闸门断链校验拦下（双保险）。

### 变更统计（构建期实测）

| 项 | 数值 |
|---|---|
| 候选池 | 115 个（声明 11 / 缩写 31 / 词组 73），**口径未变** |
| 高置信（默认档） | **42** 个 |
| 正文自带中文名 / 英文全称 | 7 / 2 条（自动提取，补全包内标注需校验） |
| 术语库 | 170 条（内置 170 · 导入 0） |
| 闸门口径 | 789 关联边 / 619 可标注文本 / 29 案例 / 19 课程 —— 与 V1.20.5 一致（**无漂移**） |

### 验证

- **闸门端到端**：造一条「生成器放行、闸门必须拦下」的导入术语（`related` 指向不存在的 id）→
  生成器导入成功 → 闸门报 `[Broken Related Terms] probe-term-v1206（imported.json）-> "no-such-term-xyz" 不存在`，
  **退出码 1**（阻断构建）；修正 `related` 后 **exit 0**（171 条 = 内置 170 + 导入 1）→ 无假阳性。
  验证后临时数据已清理，`imported.ts` 恢复 0 条。
- `tsc` 0 错误 / `lint` 0 错误 0 警告 / `check:lesson-numbers` 通过 / 闸门通过 / build 通过（238 页）。

### 待验收确认

- **P3-丁**：`zh` 自动提取在前缀复杂时可能残留噪声（实测 7 条中 1 条为「提交可疑活动报告」）。
  已刻意**只剥虚词与状语**（不剥实义动词，否则「登记机构」「报告主体」会被剥坏），
  补全包内标注「需校验」。是否值得继续加规则，取决于你对这 1/7 噪声的容忍度。
- **P3-戊**：审核页删掉 4 张统计卡后，「高置信 42 / 全部 115」只在筛选 chip 上可见；
  若你希望页面顶部保留一个「一眼看全局」的读数，可加回**一张**。
- **P3-丙**（既有，同 V1.20.5 段）：词组档精度约六成，本版以「默认只看高置信」缓解，规则本身未动。

链路规范全文见 **`docs/TERM-AUTODISCOVERY-SPEC.md`**（已更新至 V1.20.6）。
