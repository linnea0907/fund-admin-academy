# Case Library — Real Fund Admin Cases 数据规范（V2 + P1.8 + P1.8.1）
...
6. **「你的判断」与「标准答案」必须逐题一一对应**（V1.20.4 起为渲染契约，见 §七「答案区默认折叠」）。
   两种既有写法都受支持，同一案例内**不必统一**，但两小节的**题号序列必须一致**：
   - 子标题式：`## Q1` / `## Q2` …（Module 3/4/5 与 Case-026 使用）
   - 行内加粗式：`**Q1. 题干**…`（「你的判断」）/ `**Q1 — 答案首句**`（「标准答案」）（Module 1 Case-001~005 使用）

   题号数量不等或序列不一致时，逐题折叠卡片会**整体退化为整节原样渲染**（不报错、不丢内容），
   但学员就失去了「先思考再看答案」的体验 —— 导入新案例时请务必核对。
...
| 多维筛选 | V1.8.1 首屏行顺序固定：**模块（M1~M5 一级分类）→ 业务（合并后 5 类：KYC & Onboarding / AML & Compliance / Fund Structure / Fund Documents / Client Communication）→ 技能（选中业务域后动态展开该域清单）→ 难度（基础 / 进阶 / 高级，归一映射 入门·L1 / 进阶·L2 / 高级）**；**状态（待学习 / 学习中 / 已完成）与标签（Topics）收纳进默认折叠的「高级筛选」**。业务域合并规则：KYC/CDD + Investor Onboarding → KYC & Onboarding；AML + Compliance → AML & Compliance；V1.8 旧 `area` 参数（kyc-cdd / aml / investor-onboarding / compliance）经别名映射不失效。可叠加，URL 承载全部筛选维度，当前条件以可逐项移除的 chip 展示 + 一键清除。 |
...
| 答案区默认折叠 | **V1.20.4**：为「先独立思考、再看答案」，「标准答案」按题折叠为卡片 —— **题干常显 + 答案默认收起 + 每题独立展开**；`理由分析` / `常见错误` / `ICS SOP依据` 三节整节默认收起；`场景背景`/`你的判断`/`客户沟通示例`/`Takeaway` 保持展开。实现见 `src/lib/case-answers.ts`（逐题切块）+ `CaseAnswerDeck.tsx`（原生 `<details>`，无 state / 无 storage）。⚠️ 切块失败即退回整节渲染，绝不丢内容。 |
