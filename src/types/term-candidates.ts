...
export interface TermCandidate {
  ...
  cases: string[];
  /**
   * 出现的课程 id（去重，"02" / "E04"）—— 与术语数据 `courses` 字段同口径
   * （V1.20.6 起），供补全包直接填真实值。
   */
  courseIds?: string[];
  /**
   * 正文自带的中文名（V1.20.6）：来自 `中文名（缩写）` 声明，**自动提取**。
   * ⚠️ 可能有前缀噪声（如「提交可疑活动报告」），属线索而非定稿，须校验。
   */
  zh?: string;
  /** 正文自带的英文全称（V1.20.6）：来自 `缩写（英文全称）` 声明，自动提取，须校验。 */
  fullName?: string;
  /** 总出现次数 */
  count: number;
  /** 上下文样例（最多 3 条） */
